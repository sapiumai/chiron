...
def _resync_reloaded_modules():
    ...
    Several tests below reload ``chiron_cli.utils`` to re-evaluate OLLAMA_BASE_URL.
    That leaves ``chiron_cli.main``'s star-imported names (e.g. get_ticker) bound to
    ...
    importlib.reload(chiron_cli.utils)
    importlib.reload(chiron_cli.main)
...
def _reload_client():
    import chiron.llm_clients.openai_client as mod
...
# ---- chiron_cli.utils side: select_llm_provider dropdown -------------------------
...
def test_cli_dropdown_uses_env(monkeypatch):
    ...
    import chiron_cli.utils as cli_utils
...
def test_ollama_model_labels_no_local_suffix():
    ...
    from chiron.llm_clients.model_catalog import get_model_options
