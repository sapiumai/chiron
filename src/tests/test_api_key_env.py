...
from chiron.llm_clients.api_key_env import PROVIDER_API_KEY_ENV, get_api_key_env
...
def test_every_select_llm_provider_choice_has_an_entry():
    ...
    # Mirrors the dropdown order in chiron_cli/utils.select_llm_provider so the two
...
def cli_utils(monkeypatch):
    """Import chiron_cli.utils with a fresh environment so module-level state is consistent."""
    import importlib

    import chiron_cli.utils as cli_utils_module
